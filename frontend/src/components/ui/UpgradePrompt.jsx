        ...
        <div className="relative z-10 flex flex-col items-center text-center gap-3">
          ...
          <div>
            <h4 className="font-heading text-lg text-amber-400 mb-1">
              {upgradeInfo?.message || t('upgrade.premiumFeature')}
            </h4>
          ...
          >
            {t('upgrade.unlock')} {tierConfig.name} - €{tierConfig.price}{t('upgrade.perMonth')}
          </Link>
        </div>
      </div>
    );
            ...
            <div className="bg-surface-raised border border-amber-500/30 rounded-xl shadow-2xl overflow-hidden">
              ...
              <div className="bg-gradient-to-br from-amber-500/20 to-amber-600/10 p-6 text-center">
                ...
                <h2 className="text-2xl font-heading text-white mb-2">
                  {upgradeInfo?.message || t('upgrade.unlockThisFeature')}
                </h2>
              ...
              <div className="p-6">
                <h3 className="text-sm font-heading text-text-dim uppercase tracking-wide mb-3">
                  {tierConfig.name} {t('upgrade.includes')}
                </h3>
                <ul className="space-y-2 mb-6">
                  {tierConfig.features.slice(0, 5).map((f, i) => (
                    <li key={i} className="flex items-center gap-2 text-sm">
                      <span className="text-green-500">✓</span>
                      <span className="text-text">{formatFeatureName(f, t)}</span>
                    </li>
                  ))}
                ...
                <div className="space-y-3">
                  ...
                  >
                    {t('upgrade.viewPlansFrom')} €{tierConfig.price}{t('upgrade.perMonth')}
                  </Link>
    ...
    return (
      <div className="flex items-center gap-2 p-3 bg-red-500/10 border border-red-500/30 rounded-lg">
        ...
        <span className="text-sm text-red-400">
          {t('upgrade.reachedDailyLimit')}{' '}
          <Link to="/pricing" className="underline ml-1">{t('upgrade.upgradeForUnlimited')}</Link>
        </span>
      </div>
    );
  }
  ...
  if (isLow) {
    return (
      <div className="flex items-center gap-2 text-amber-400 text-sm">
        ...
        <span>
          {remaining === 1 
            ? t('upgrade.remainingToday', { count: remaining })
            : t('upgrade.remainingTodayPlural', { count: remaining })}
        </span>
      </div>
    );
  }
  ...
  if (percentUsed > 50) {
    return (
      <div className="text-xs text-text-dim">
        {t('upgrade.usesRemainingToday', { count: remaining })}
...
function formatFeatureName(feature, t) {
  const names = {
    'synergy_full': t('upgrade.synergyFullAnalysis'),
    'synergy_suggestions': t('upgrade.itemSuggestions'),
    'optimal_path': t('upgrade.optimalPathSuggestions'),
    'save_parser_full': t('upgrade.fullSaveParser'),
    'builds_unlimited': t('upgrade.unlimitedBuilds'),
    'favorites_unlimited': t('upgrade.unlimitedFavorites'),
    'export_data': t('upgrade.exportData'),
    'no_ads': t('upgrade.noAds'),
    'profile_customization': t('upgrade.profileCustomization'),
    'profile_badge': t('upgrade.exclusiveBadge'),
    'early_access': t('upgrade.earlyAccess'),
    'priority_support': t('upgrade.prioritySupport')
  };
